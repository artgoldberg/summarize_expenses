...
class Test(unittest.TestCase):
    ...
    def test(self):
        for debug in [False, True]:
            for taxes in [False, True]:
                args = Namespace(data_dir=None, debug=debug, files=[self.DATA_FILENAME], filter=None,
                    select=None, taxes=taxes)
                tax_expenses, spending_expenses = summarize_expenses.collect_tax_data.main(args)
        ...
        with open(self.SELECTORS, 'r') as selector:
            ...
            tax_expenses, _ = summarize_expenses.collect_tax_data.main(args)
        ...
        with self.assertRaises(ValueError):
            summarize_expenses.collect_tax_data.main(args)
